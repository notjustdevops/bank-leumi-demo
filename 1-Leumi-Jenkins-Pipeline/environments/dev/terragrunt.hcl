...
locals {
  ...
  dynamodb_table_for_locks = "dvorkinguy-leumi-jenkins-terraform-locks"
  common_tags = {
    Name        = "Leumi-Jenkins"
    Owner       = "Dvorkin Guy"
    Environment = "Dev"
...
inputs = {
  ...
  # Pass VPC outputs to other modules
...
dependency "security" {
  ...
  outputs = ["security_group_id"]  # Capture the security group output if needed by other modules
}
